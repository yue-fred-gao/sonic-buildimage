...
struct ocores_i2c {
    ...
    int tmp_state; /* for STATE_DONE and STATE_ERROR */
...
static void ocores_process(struct ocores_i2c *i2c, u8 stat)
{
    ...
    if ((i2c->tmp_state == STATE_DONE) || (i2c->tmp_state == STATE_ERROR)) {
        /* stop has been sent */
        oc_setreg(i2c, OCI2C_CMD, OCI2C_CMD_IACK);
        i2c->state = i2c->tmp_state;
        wake_up(&i2c->wait);
               ...
               const unsigned long timeout)
{
    ...
    j = jiffies + timeout;
    while (1) {
        FPGA_I2C_DEBUG_VERBOSE("Read Status register to check if the ocores status is ok, mask: 0x%02x, except value: 0x%02x\n",
            mask, val);
        jiffies_tmp = jiffies;
        status = oc_getreg(i2c, reg);
        if ((status & mask) == val) {
            FPGA_I2C_DEBUG_VERBOSE("Ocores wait ok, ocores status: 0x%02x, mask: 0x%02x, except value: 0x%02x\n",
                status, mask, val);
            break;
        }

        FPGA_I2C_DEBUG_VERBOSE("Ocores wait retry, ocores status: 0x%02x, mask: 0x%02x, except value: 0x%02x\n",
            status, mask, val);

        if (time_after(jiffies_tmp, j)) {
            FPGA_I2C_DEBUG_VERBOSE("Ocores wait timeout, ocores status: 0x%02x, mask: 0x%02x, except value: 0x%02x\n",
                status, mask, val);
            return -ETIMEDOUT;
        }
...
static void ocores_check_i2c_unblock(struct ocores_i2c *i2c)
{
    u8 stat, ctrl;
    unsigned long flags;
    int err;
    ...
    stat = oc_getreg(i2c, OCI2C_STATUS);
    if (!(stat & OCI2C_STAT_ARBLOST)) {
        FPGA_I2C_DEBUG_XFER("ocores nr: %d, status: 0x%x, not Arbitration lost\n", i2c->adap.nr, stat);
        FPGA_I2C_SPIN_UNLOCK(i2c->process_lock, flags);
        return;
    }
    FPGA_I2C_DEBUG_XFER("ocores nr: %d, status: 0x%x, Arbitration lost, try to send 9 clock\n",
        i2c->adap.nr, stat);
    ...
    if (err) {
        FPGA_I2C_DEBUG_XFER("ocores nr: %d, wait  transfer complete timeout in %dms, err: %d\n",
            i2c->adap.nr, I2C_WAIT_RD_STOP_TIMEOUT_MS, err);
    }
...
static int ocores_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
    ...
    xfer_ret = i2c->state;
    FPGA_I2C_MUTEX_UNLOCK(i2c->xfer_lock);
    return (xfer_ret == STATE_DONE) ? num : -EAGAIN;
}
...
static const struct of_device_id ocores_i2c_match[] = {
    {
        .compatible = "opencores,mc-i2c-ocores",
...
/* END */
static int mc_ocores_i2c_probe(struct platform_device *pdev)
{
    struct ocores_i2c *i2c;
    struct mc_ocores_i2c_platform_data *pdata;
    struct resource *res;
    int irq;
    int ret;
    int i;
...
static int mc_ocores_i2c_remove(struct platform_device *pdev)
#else
static void mc_ocores_i2c_remove(struct platform_device *pdev)
#endif
{
...
static struct platform_driver ocores_i2c_driver = {
    .probe   = mc_ocores_i2c_probe,
    .remove  = mc_ocores_i2c_remove,
    .driver  = {
        .owner = THIS_MODULE,
        .name = "mc-i2c-ocores",
