...
static int firmware_fpga_set_access(firmware_spi_logic_upg_t *upg_priv, uint32_t cmd)
{
    ...
    if (ret) {
        ...
        addr = upg_priv->spi_ctrl_reg;
        val = FPGA_UPG_ACCESS_DISABLE;
        firmware_fpga_write_word(upg_priv->dev_path, addr, val);
...
/* Get flash Device_id */
static int firmware_fpga_get_flash_id(firmware_spi_logic_upg_t *upg_priv, uint32_t *flash_id)
{
    int ret;
    uint32_t val, addr, cmd;
...
int firmware_upgrade_spi_logic_dev_test(int fd, name_info_t *info)
{
    ...
    dbg_print(is_debug_on, "current_upg_priv dev_path[%s] ctrl_base[0x%0x] test_base[0x%0x] test_size[0x%x]\n",
                current_upg_priv.dev_path, current_upg_priv.ctrl_base, current_upg_priv.test_base, current_upg_priv.test_size);
        ...
        uint32_t len, char *record_file)
{
    ...
    if (strcmp(record_file, "print") != 0) {      /* record dump data on 'record_file' */
        ...
        snprintf(save_file, sizeof(save_file), "%s", record_file);
...
int firmware_upgrade_spi_logic_dev_get_flash_id(char *dev_name, uint32_t *flash_id)
{
    int ret, fd, retry;
    firmware_spi_logic_upg_t current_upg_priv;
    ...
    dbg_print(is_debug_on, "current_upg_priv dev_path[%s] flash_base[0x%0x] ctrl_base[0x%0x], test_base[0x%x], test_size[0x%x]\n",
                current_upg_priv.dev_path, current_upg_priv.flash_base,
                current_upg_priv.ctrl_base, current_upg_priv.test_base, current_upg_priv.test_size);
    ...
    /* read flash chip id  */
    for (retry = 0; retry < FPGA_UPG_RETRY_TIMES; retry++) {
        ret = firmware_fpga_get_flash_id(&current_upg_priv, flash_id);
        if ((ret < 0) || (*flash_id == 0xffffff)) {
            dbg_print(is_debug_on, "read flash id failed, ret: %d,  flash_id: 0x%x, retry: %d\n", ret, *flash_id, retry);
            continue;
        }
        break;
    }

    if (retry == FPGA_UPG_RETRY_TIMES) {
        dbg_print(is_debug_on, "Error:firmware_fpga_get_flash_id failed ret %d, flash_id: 0x%x\n", ret, *flash_id);
        goto fail;
    }
